  ...
  return (
    <div className='flex flex-col items-center'>
      <h2 className='my-10 sm:my-16 text-3xl sm:text-6xl lg:text-7xl text-white text-center font-extrabold'>
        The only URL Snipper <br/> you&rsquo;ll ever need! 👇
      </h2>
      <form className='sm:h-14 flex flex-col sm:flex-row w-full md:w-2/4 gap-2' onSubmit={handleShorten}>
        <Input type='url' value={longUrl} placeholder='Enter your looong URL' onChange={(e)=>setLongUrl(e.target.value)} className='h-full flex-1 px-4 py-4'/>
        <Button className='h-full' type='submit' variant='destructive'>Shorten!</Button>
      </form>
      <img src='banr.webp' alt='banner' className='w-full my-11 md:px-11'/>
      <Accordion type="multiple" collapsible="true" className='w-full md:px-11'>
        <AccordionItem value="item-1">
        <AccordionTrigger>How does 'url Snipper' shortener works?</AccordionTrigger>
        <AccordionContent>
        ...
        <AccordionContent>
          You can view the number of clicks, geolocation data of the clicks and device types (mobile/desktop)
