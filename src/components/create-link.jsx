...
const CreateLink = () => {
    ...
    const createNewUrl = async () => {
        ...
        try {
            ...
            const blob = await new Promise((resolve) => canvas.toBlob(resolve)); //qr in the blob
        ...
        <Dialog defaultOpen={longLink} onOpenChange={(res) => {
            ...
            <DialogContent className='sm:max-w-md'>
                ...
                <div className='flex items-center gap-2'>
                    <Card className='p-2'>urlSnipper.in</Card> /
                    <Input id='customUrl' placeholder="Custom Link (optional)" value={formValues.customUrl} onChange={handleChange} />
                </div>
                {error && <Error message={error.message} />}
                <DialogFooter className='sm:justify-start'>
                    <Button disabled={loading} onClick={createNewUrl} variant='destructive' >{loading ? <BeatLoader size={10} color='white' /> : 'Create'}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    )
